...
metas_bp = Blueprint(
    'metas', __name__, url_prefix='/metas',
    template_folder='../templates', static_folder='../static'
)

PROJECT_DIR = Path(__file__).resolve().parent.parent
JSON_REL_PATH = Path("conexiones") / "data_ops" / "metas.json"
...
def normalize_entry(raw):
    """
    Normaliza keys a: "Centro Costos", "Material", "Meta Cantidad"
    """
    ...
    meta = raw.get("Meta Cantidad") or raw.get("meta cantidad") or raw.get("Meta") or raw.get("meta") or 0
    try:
        if isinstance(meta, str):
            meta = meta.strip()
            meta = int(float(meta)) if meta != "" else 0
        else:
            meta = int(meta)
    except Exception:
        meta = 0
...
def _load_existing_centros():
    ...
    for item in data:
        if not isinstance(item, dict):
            continue
        c = item.get("Centro Costos") or item.get("Centro") or item.get("centro") or ""
        if c is not None:
            cc = str(c).strip()
            if cc:
                cents.add(cc)
...
# API: listar
@metas_bp.route('/api/items', methods=['GET'])
def api_list():
    items = read_metas()
    return jsonify(items), 200

# API: crear (permitir duplicados)
@metas_bp.route('/api/items', methods=['POST'])
def api_create():
...
@metas_bp.route('/api/items/<int:index>', methods=['PUT'])
...
@metas_bp.route('/api/items/<int:index>', methods=['DELETE'])
...
@metas_bp.route('/api/delete_all', methods=['POST'])
def api_delete_all():
...
@metas_bp.route('/api/export', methods=['GET'])
def api_export():
    fmt = request.args.get("format", "excel").lower()
    items = read_metas()
    df = pd.DataFrame(items)
    for c in ["Centro Costos", "Material", "Meta Cantidad"]:
        if c not in df.columns:
            df[c] = ""
    df = df[["Centro Costos", "Material", "Meta Cantidad"]]
    if fmt in ("excel", "xlsx"):
        ...
        mem.seek(0)
        return send_file(mem, as_attachment=True, download_name="metas.json", mimetype="application/json")

# API: importar (Excel o JSON)
@metas_bp.route('/api/import', methods=['POST'])
def api_import():
    ...
    try:
        if filename.endswith(('.xls', '.xlsx')) or content[:4] == b'PK\x03\x04':
            ...
            col_meta = get_col(["Meta Cantidad", "meta cantidad", "Meta", "meta", "Cantidad", "cantidad", "Qty", "qty"])
            if not col_material:
            ...
            for _, row in df.iterrows():
                ...
                meta = row.get(col_meta) if col_meta else 0
                try:
                    meta = int(float(str(meta))) if str(meta).strip() != "" else 0
                except Exception:
                    meta = 0
                to_add.append({
...
@metas_bp.route('/api/pending', methods=['GET'])
def api_pending():
    items = read_metas()
