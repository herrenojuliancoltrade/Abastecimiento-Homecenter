// static/js/transitos.js
const API_BASE = "/transitos/api";
...
function renderTable(items) {
  ...
  items.forEach((it, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>${it["Material"] || ""}</td>
      <td>${it["Transitos"] != null ? it["Transitos"] : ""}</td>
      <td>
...
async function onEdit(e) {
  ...
  $("#input-transitos").value = item["Transitos"] != null ? item["Transitos"] : 0;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // si no existe aún en DOM (por diferencias) falla silenciosamente
  ...
  $("#item-form").addEventListener("submit", async (ev) => {
    ...
    const transitos = Number($("#input-transitos").value || 0);
    const editing = $("#editing-index").value;
    const payload = { "Centro Costos": centro, "Material": material, "Transitos": transitos };
